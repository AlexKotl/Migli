	...
	if ($_SESSION[access_level]==1) {
		...
		if ($_GET[action]=='routing') {
			...
			elseif (isset($_REQUEST[submit_but])) {
				...
				//$res = $db->query("select * from subscribe where flag=1 and email like 'kat_r%' order by id desc ");
				//while ($row=$db->fetch($res)) {
				foreach ($_REQUEST[checkboxes] as $v) {
					...
					$sys_message .= "Отправлено на email: $v<br>";
				}
				...
				$content .= "<form class='form-horizontal' action='?module=$module&action=routing' method='post' enctype='multipart/form-data'>
				  ...
				  <div class='control-group'>
				    ...
				    <div class='controls'>
				      <textarea name='mail_content' style='height:300px' placeholder='Можно использовать HTML: <b>жирный</b>, <i>курсив</i>, <u>подчеркнутый</u>, <a href=\"http...\">ссылка</a>, <img src=\"http://...\">'></textarea>
				    </div>
				  </div>
				  <div class='control-group'>
				    <div class='controls'>
		...
		// LIST VIEW
		if ($_GET[action]=='' || $_GET[action]=='write' || ($_GET[action]=='routing' && !isset($_REQUEST[submit_but]) && !isset($_REQUEST[preview_but]))) {
			
			$content .= "
				<table class='table table-striped table-hover table-bordered'>
				<thead><tr><th><input type='checkbox' class='selectAll'></th><th>#</th><th>Email</th><th>Name</th><th colspan=2></th></tr></thead><tbody>";
				
			
			$res = $db->query("select * from subscribe order by id desc ");
			while ($row=$db->fetch($res)) {
				$content .= "<tr ".($row[flag]==2 ? "class='error'" : '').">
					<td><input type='checkbox' name='checkboxes[]' value='{$row[email]}'></td>
					<td>$row[id]</td>
				    <td>$row[email]</td>
				    <td>$row[name]</td>
				    <td>
